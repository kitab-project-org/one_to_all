...
def download_srt_files(base_url, main_text_id, outfolder, stats_folder, incl_sec=False):
...
def extract_milestone_data_from_folder(folder, outfolder, stats_folder, openiti_version, meta,
...
##                        rows.append(",".join([str(x) for x in row]))
        ...
        stats_fp = os.path.join(stats_folder, "{}_{}_stats.csv".format(openiti_version, main))
        ...
        stats_fp = os.path.join(stats_folder, "{}_{}_stats.json".format(openiti_version, main))
        with open(stats_fp, mode="w", encoding="utf-8") as file:
...
for i, id_ in enumerate(os.listdir(base_url)):
    srt_folder = os.path.join(base_url, id_)
    if not os.path.isdir(srt_folder):
        continue
    print(i, id_)
    start = time.time()
    extract_milestone_data_from_folder(srt_folder, outfolder, stats_folder,openiti_version, meta,
                                   single_json=True, csv_output=True,
